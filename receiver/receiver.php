			...
			if ($pid) {
				...
				foreach ($quad as $q) {
					...
					if (empty($l)) {
						...
						`DISPLAY=:0 /usr/bin/ffplay -protocol_whitelist file,udp,rtp -i /tmp/{$id}.sdp -probesize 65536 -sync ext -loglevel quiet -stats -hide_banner -fflags nobuffer -flags low_delay -framedrop -reorder_queue_size 0 -left {$q[0]} -top {$q[1]} -window_title "Stream: {$id}" -x {$bw} -y {$bh} -noborder > /dev/null 2>&1 &`;
